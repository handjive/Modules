...
function EverythingAPIFactory()
{
    $esapi =&{
        return (add-type -memberDefinition $esapi_cscode -name "YetAnotherEverything" -passthru)
    }
...
class EverythingAPI
{
    [object]static $substance
    [object]static DefaultAPI()
    {
        if( $null -eq [EverythingAPI]::substance ){
            Write-Output 'Initializing EverythingAPI.'
            [EverythingAPI]::substance = EverythingAPIFactory
        }
        return ([EverythingAPI]::substance)
    }
}
